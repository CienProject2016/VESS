...
void BackgroundObject::setSpeed(float speed) {
	accel = (speed - settingSpeed)/accelTime;
...
void BackgroundObject::update(float dt) {
	...
	if((accel > 0 && speed < settingSpeed) || (accel < 0 && speed > settingSpeed))
		speed += accel * dt;
