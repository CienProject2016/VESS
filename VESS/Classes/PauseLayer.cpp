...
void PauseLayer::resumeButtonCallback(cocos2d::Ref* pSender, ui::Widget::TouchEventType type) {
	...
	case ui::Widget::TouchEventType::ENDED: {
			...
			/*
			removeChild(getChildByName("grayLayer"));
			removeChild(pauseBackground);
			removeChild(effectSoundBackground);
			removeChild(bgmSoundBackground);
			removeChild(restartGameButton);
			removeChild(resumeGameButton);
			removeChild(bgmSoundButton);
			removeChild(effectSoundButton);*/
			//Director::getInstance()->resume();
